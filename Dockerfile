...
RUN yum install -y epel-release && rpm -ivh http://yum.postgresql.org/9.4/redhat/rhel-7-x86_64/pgdg-centos94-9.4-1.noarch.rpm  \
                                && yum -y install --setopt=tsflags=nodocs nodejs tar sudo git memcached postgresql94-devel postgresql94-server \
                                          libxml2-devel libxslt-devel patch gcc-c++ openssl-devel gnupg curl which net-tools libyaml-devel \
                                          autoconf readline-devel libffi-devel bzip2 automake libtool bison sqlite-devel cmake; yum clean all

ENV PATH $PATH:/usr/pgsql-9.4/bin/:/usr/local/rvm/gems/ruby-2.2.3/bin/:/usr/local/rvm/bin
...
# Required to allow systemd to run in non-privileged container. It is required to mount it as a volume
# docker run -d -ti -v /sys/fs/cgroup:/sys/fs/cgroup:ro -p 3000:3000 <image_name>
...
RUN /usr/bin/curl -sSL https://rvm.io/mpapis.asc | gpg2 --import - && /usr/bin/curl -sSL https://get.rvm.io \
                                                 | rvm_tar_command=tar bash -s stable && source /etc/profile.d/rvm.sh \
                                                 && echo "gem: --no-ri --no-rdoc --no-document" > ~/.gemrc \
                                                 && /bin/bash -l -c "rvm requirements ; rvm install ruby 2.2.3 ; rvm use 2.2.3 --default ; \
...
RUN    su postgres -c "pg_ctl -D /var/lib/pgsql/data start" \
    ...
    && /bin/bash -l -c "bin/setup" && su postgres -c "pg_ctl -D /var/lib/pgsql/data stop" \
    && pkill memcached && /bin/bash -l -c "rvm cleanup all ; rm -vf $(/usr/local/rvm/bin/rvm gemdir)/cache/* ; rvm disk-usage all"
...
RUN sed -i s#PGDATA=/var/lib/pgsql/9.4/data/#PGDATA=/var/lib/pgsql/data/#g /usr/lib/systemd/system/postgresql-9.4.service
